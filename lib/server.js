...
Server.prototype.resizeImage = function (src, dest, parameters, callback) {
    ...
    fs.stat(src, function handleOriginalStat(err, size) {
        ...
        if (parameters.type === self.imgr.FILL && parameters.width && parameters.height) {
            self.options.trace('resize_fill');
            imgr.fillResize(parameters.width, parameters.height, parameters.orientation);
...
Server.prototype.middleware = function () {
    var namespace = this.options.namespace.replace(/\/$/, '')
      ...
      , doetag = this.options.etag !== false
      , setHeaders = typeof this.options.setHeaders === 'function' ? this.options.setHeaders : function() {}
      , info = this.info.bind(this)
      , self = this;

    //Setup the static servers
    var base_static = express.static(base_dir, { maxAge: maxAge, etag: doetag, setHeaders: setHeaders })
      , cache_static = base_static;
    if (cache_dir !== base_dir) {
        cache_static = express.static(cache_dir, { maxAge: maxAge, etag: doetag, setHeaders: setHeaders });
    }
    ...
    function tryContentDirectory(request, response, next) {
        info('trying to serve content %s', path.join(base_dir, request.url));
        base_static(request, response, next);
    }

    function tryCacheDirectory(request, response, next) {
        info('trying to serve cache %s', path.join(cache_dir, request.url));
...
Server.prototype.compileRegexp = function (pattern) {
    ...
    pattern = pattern.replace(':size', '(\\d+x|x+\\d+|\\d+x\\d+(?:-[a-z]+){0,2})')
                     .replace('/:path', '(?:/(.+?))?')
                     .replace(':ext', '([^/]+?)')
                     .replace(':file', '([^/]+?)');
    ...
    return function compileRegexp(url) {
        ...
        if (parsed.size) {
            ...
            result.height = (result.width[1] || '').split('-', 3);
            result.orientation = result.height[1] || null;
            result.type = result.height[2] || null;
            ...
            if (result.type) {
                switch (result.type) {
                case 'fill':
                    result.type = imgr.FILL;
                    break;
                default:
                    result.type = imgr.CROP;
                    break;
                }
            }

            info('result type %s', result.type);
        }
        return result;
    };
};
